...
export interface DetectedPattern {
  type: 'TRANSACTION_CHAIN' | 'COMMUNICATION_SPIKE' | 'SHARED_INFRASTRUCTURE' | 'CROSS_COMMUNITY';
  title: string;
  description: string;
...
export class PatternDetectorService {
   ...
   */
  static async detectAllPatterns(caseId: string): Promise<DetectedPattern[]> {
   ...
   */
  static async detectTransactionChains(caseId: string): Promise<DetectedPattern[]> {
          ...
          for (const t3 of thirdHops) {
            ...
            if (timeDiff2 >= 0 && timeDiff2 <= 48) {
              ...
              if (!visitedChains.has(chainKey)) {
                ...
                const accNodes = await prisma.graphNode.findMany({
                  where: {
                    caseId,
                    entityType: 'ACCOUNT',
                    canonicalValue: {
                ...
                const evidenceIds = [t1.evidenceRecordId, t2.evidenceRecordId, t3.evidenceRecordId].filter(
                  Boolean
                ) as string[];
   ...
   */
  static async detectCommunicationSpikes(caseId: string): Promise<DetectedPattern[]> {
    ...
    // Group by pair & date
    const pairDailyCounts = new Map<string, { [dateKey: string]: { count: number; evidenceIds: string[] } }>();
    ...
    for (const c of calls) {
      ...
      if (c.evidenceRecordId) pairData[dateKey].evidenceIds.push(c.evidenceRecordId);
    }
    ...
    for (const [pair, dayMap] of pairDailyCounts.entries()) {
      ...
      for (const day of days) {
        ...
        if (dayCount >= 3 && (dayCount >= baselineOthers * 2.0 || otherDaysCount === 0)) {
          ...
          const phoneNodes = await prisma.graphNode.findMany({
   ...
   */
  static async detectSharedInfrastructure(caseId: string): Promise<DetectedPattern[]> {
    const patterns: DetectedPattern[] = [];

    // Find non-person nodes connected to >= 2 distinct PERSON nodes
    const infraNodes = await prisma.graphNode.findMany({
      where: {
        caseId,
    ...
    for (const node of infraNodes) {
      ...
      const linkedPersonNames: string[] = [];

      const checkNode = (other: any) => {
