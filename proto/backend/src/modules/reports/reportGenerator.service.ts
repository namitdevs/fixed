...
import { GraphAnalyticsService } from '../analytics/graphAnalytics.service';
...
export class ReportGeneratorService {
  static async generateCaseReport(caseId: string, authorUserId?: string) {
    ...
    // Clusters
    const clusters = await GraphAnalyticsService.getCommunityClusters(caseId);

    // Alerts & Patterns
    const alerts = await prisma.alert.findMany({ where: { caseId }, orderBy: { severity: 'asc' } });
    ...
    const executiveSummary =
      `This confidential investigative dossier synthesizes analytical findings for ${caseData.caseNumber} ("${caseData.title}"). ` +
  ...
  static async streamPdfReport(caseId: string, res: Response): Promise<void> {
    const caseData = await prisma.case.findUnique({
      where: { id: caseId },
    ...
    const alerts = await prisma.alert.findMany({ where: { caseId }, take: 6 });
    const timeline = await TimelineService.getCaseTimeline(caseId);
    ...
    if (typeof res.setHeader === 'function') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="Investigation_Dossier_${caseData.caseNumber}.pdf"`
      );
    }
    ...
    doc.fontSize(10).fillColor('#334155').text(
      `Analytical assessment of Case ${caseData.caseNumber} indicates a structured criminal syndicate organized across multiple functional tiers. ` +
    ...
    topMetrics.forEach((m, idx) => {
      doc.font('Helvetica-Bold').fontSize(10).fillColor('#0f172a').text(
        `${idx + 1}. ${m.graphNode.canonicalValue} [${m.graphNode.entityType}] — Network Score: ${m.networkRelevanceScore}/100`
      );
      doc.font('Helvetica').fontSize(9).fillColor('#64748b').text(
        `   • Connectivity: ${m.degreeCentrality} direct links | Betweenness Rank: ${m.betweennessCentrality} | Bridge Node: ${m.isPotentialBridge ? 'YES' : 'NO'}`
      );
      const factors: any[] = m.relevanceFactors ? JSON.parse(m.relevanceFactors) : [];
    ...
    doc.fontSize(8).fillColor('#94a3b8').text(
      'LEGAL DISCLAIMER: This automated intelligence assessment is intended exclusively for investigatory prioritization and decision support. It does not constitute a legal determination of guilt or criminal indictment.',
      { align: 'center' }
    );
  ...
  static async getJsonExport(caseId: string) {
    const caseData = await prisma.case.findUnique({
      where: { id: caseId },
      include: {
        documents: true,
  ...
  static async getCsvExport(caseId: string) {
