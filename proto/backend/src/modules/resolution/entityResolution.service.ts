...
export class EntityResolutionService {
   ...
   */
  static async runResolution(caseId: string): Promise<number> {
    const personNodes = await prisma.graphNode.findMany({
    ...
    for (let i = 0; i < personNodes.length; i++) {
      for (let j = i + 1; j < personNodes.length; j++) {
        ...
        if (matchResult.isCandidate) {
          ...
          const existing = await prisma.entityResolutionCandidate.findFirst({
            where: {
              caseId,
              OR: [
  ...
  static async listCandidates(caseId: string, status?: string) {
    ...
    const populated = await Promise.all(
      candidates.map(async (c) => {
        const primary = await prisma.graphNode.findUnique({ where: { id: c.primaryNodeId } });
        const candidate = await prisma.graphNode.findUnique({ where: { id: c.candidateNodeId } });
        return {
          ...c,
  ...
  static async resolveCandidate(candidateId: string, action: 'ACCEPT' | 'REJECT', userId?: string): Promise<{ success: boolean; message: string; candidate?: any }> {
    const candidate = await prisma.entityResolutionCandidate.findUnique({
      where: { id: candidateId },
    });

    if (!candidate) {
      const err: any = new Error(`Resolution candidate ${candidateId} not found`);
      err.statusCode = 404;
    ...
    const primary = await prisma.graphNode.findUnique({ where: { id: candidate.primaryNodeId } });
    const secondary = await prisma.graphNode.findUnique({ where: { id: candidate.candidateNodeId } });
    ...
    const updatedCandidate = await prisma.entityResolutionCandidate.update({
      where: { id: candidateId },
      data: {
    ...
    await prisma.graphNode.delete({ where: { id: secondary.id } });
  ...
  private static evaluateMatch(
    nodeA: any,
    nodeB: any
  ): { isCandidate: boolean; score: number; reasons: string[] } {
    ...
    const rawsA: string[] = nodeA.rawValues ? JSON.parse(nodeA.rawValues) : [];
    const rawsB: string[] = nodeB.rawValues ? JSON.parse(nodeB.rawValues) : [];
  ...
  private static isInitialsMatch(a: string, b: string): boolean {
