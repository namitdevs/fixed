...
exports.RelevanceScorerService = void 0;
...
class RelevanceScorerService {
     ...
     */
    static async runFullIntelligence(caseId) {
        const startTime = Date.now();
        // 1. Create AnalysisRun record
        const run = await prisma_1.prisma.analysisRun.create({
            data: {
                caseId,
        ...
        const centralityList = await graphAnalytics_service_1.GraphAnalyticsService.runAnalytics(caseId);
        ...
        for (const c of centralityList) {
            ...
            for (const p of relatedPatterns) {
                let pts = 15;
                if (p.type === 'TRANSACTION_CHAIN')
                    pts = 25;
                if (p.type === 'COMMUNICATION_SPIKE')
                    pts = 20;
                factors.push({
                    factor: p.type,
                    contribution: pts,
        ...
        const nodeCount = await prisma_1.prisma.graphNode.count({ where: { caseId } });
        const edgeCount = await prisma_1.prisma.graphEdge.count({ where: { caseId } });
        await prisma_1.prisma.analysisRun.update({
            where: { id: run.id },
            data: {
                status: 'COMPLETED',
                nodeCount,
    ...
    static async getNodeScore(graphNodeId) {
        const metric = await prisma_1.prisma.nodeMetric.findFirst({
            where: { graphNodeId },
...
exports.RelevanceScorerService = RelevanceScorerService;
