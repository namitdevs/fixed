...
class RelationshipExtractor {
     ...
     */
    static async extractRelationships(caseId) {
        let edgeCount = 0;
        ...
        const nodeMap = new Map(); // "TYPE:VALUE" -> nodeId
        ...
        for (const ph of phones) {
            const person = await prisma_1.prisma.person.findUnique({ where: { id: ph.ownerPersonId } });
            if (!person)
                continue;
        ...
        for (const v of vehicles) {
            const person = await prisma_1.prisma.person.findUnique({ where: { id: v.ownerPersonId } });
            if (!person)
                continue;
            const personNodeId = nodeMap.get(`PERSON:${person.canonicalName}`);
        ...
        for (const c of calls) {
            ...
            const callerOwner = await prisma_1.prisma.phone.findFirst({
                where: { caseId, canonicalNumber: c.callerPhone },
            });
            const receiverOwner = await prisma_1.prisma.phone.findFirst({
                where: { caseId, canonicalNumber: c.receiverPhone },
            });
            if (callerOwner?.ownerPersonId && receiverOwner?.ownerPersonId) {
                const p1 = await prisma_1.prisma.person.findUnique({ where: { id: callerOwner.ownerPersonId } });
                const p2 = await prisma_1.prisma.person.findUnique({ where: { id: receiverOwner.ownerPersonId } });
                if (p1 && p2) {
    ...
    static async extractNarrativeRelationships(caseId, nodeMap) {
    ...
    static async upsertEdge(data) {
        if (data.sourceNodeId === data.targetNodeId)
            return null;
        const existing = await prisma_1.prisma.graphEdge.findFirst({
            where: {
                caseId: data.caseId,
        ...
        if (existing) {
            return await prisma_1.prisma.graphEdge.update({
        ...
        return await prisma_1.prisma.graphEdge.create({
            data: {
                caseId: data.caseId,
                sourceNodeId: data.sourceNodeId,
                targetNodeId: data.targetNodeId,
